...
module.exports = class ScheduleForecaster {
    constructor(opts) {
        this.nextTime = new NextTime(opts);
    }
    ...
    forecastSchedule(args) {
        ...
        switch (args.recurrence) {
            ...
            case 'weekly':
                ...
                break;
            case 'monthly':
                let recurrenceDates = [startDate];
                const s = startDate.getDate(); // Starting date
                let nextMonth = NextTime.nextMonth(startDate, s);
                while (nextMonth < endDate) {
                    recurrenceDates.push(nextMonth);
                    nextMonth = NextTime.nextMonth(nextMonth, s);
                }
                occurences = recurrenceDates.length;
                break;
            default:
                occurences = 0;
                break;
